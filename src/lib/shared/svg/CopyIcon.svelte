<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class={$$props.class}
	><!--! Font Awesome Pro 6.1.2 by @fontawesome - https://fontawesome.com License - https://fontawesome.com/license (Commercial License) Copyright 2022 Fonticons, Inc. --><defs
	/><path
		class="fa-primary opacity-100 fill-zinc-600"
		d="M192 352V128h-144c-26.51 0-48 21.49-48 48v288c0 26.51 21.49 48 48 48h192c26.51 0 48-21.49 48-48L288 416h-32C220.7 416 192 387.3 192 352zM416 0v96h96L416 0z"
	/><path
		class="fa-secondary stroke-[1rem] stroke-cyan-600 fill-transparent opacity-100"
		d="M512 96v240c0 26.51-21.49 48-48 48h-192c-26.51 0-48-21.49-48-48v-288c0-26.51 21.49-48 48-48H416v96H512z"
	/></svg
>
